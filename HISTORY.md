...
- Extends the canonical `mech_marketplace` contract wrapper (`packages/valory/contracts/mech_marketplace/contract.py`) with two additive read methods that off-chain-request paths need to derive `request_id` client-side: `get_chain_id()` reads `chainId()` for the EIP-712 domain binding, and `get_nonce(sender)` reads `mapNonces(sender)` for the per-requester nonce slot. Both return the framework's standard `{"data": ...}` shape, matching the sibling `get_balance_tracker_for_mech_type` and `get_fee` views. No existing method is renamed or resignatured; downstream consumers pick this up via a routine `third_party` hash bump at their next re-lock. #2537

# 0.21.25 (2026-06-24)

Framework / dependency bumps:

- Bumps `open-aea` `2.2.8` → `2.2.9` across `pyproject.toml`, `setup.py`, `tox.ini`, `skaffold.yaml`, both Dockerfiles' `AEA_VERSION` arg, `deployments/Dockerfiles/autonomy-user/requirements.txt`, the 33 package YAMLs that pin `open-aea-*` dependencies, the two doc snippets (`docs/guides/define_agent.md`, `docs/advanced_reference/commands/autonomy_build-image.md`), and `CLAUDE.md`. Pulls in [open-aea 2.2.9](https://github.com/valory-xyz/open-aea/releases/tag/v2.2.9): `EthereumApi.build_transaction` now propagates the sender as `from` in the built `tx_params`, so `eth_estimateGas` simulates with the actual sender instead of `msg.sender = 0x0` — fixing spurious pre-flight reverts on ERC20 / marketplace flows that branch on `msg.sender` ([#925](https://github.com/valory-xyz/open-aea/pull/925)). No API or wire-format changes; `autonomy/constants.py` `ABSTRACT_ROUND_ABCI_SKILL_WITH_HASH` resynced to the relocked IPFS hash; package hashes regenerated via `autonomy packages lock` and `poetry.lock` regenerated to match. #2535
...
- Caps `RetriesInfo.suggested_sleep_time` at `MAX_SUGGESTED_SLEEP_TIME` (3600 s) to prevent `OverflowError` in `BaseBehaviour.sleep` when an agent retries against a persistently failing remote (with the default `backoff_factor=2.0`, the raw exponential overflows `datetime.timedelta` around retry ~38). A single `WARNING` log is emitted per `RetriesInfo` the first time the cap engages, latched so subsequent reads within the same retry sequence stay silent; the latch resets automatically once the raw value drops back under the cap (e.g. after `reset_retries`). No API changes (#2520).
- Fixes two crashes where an unguarded read of `RoundSequence.last_round_transition_timestamp` terminated the agent before the first round transition of a period had completed. The property deliberately raises `ValueError` until a transition completes, and `RoundSequence.reset_state` resets `_last_round_transition_timestamp` to `None` during an agent <-> Tendermint communication recovery: (a) `BaseBehaviour._get_reset_params` then crashed the subsequent `ResetAndPauseBehaviour` hard reset, and (b) `SlashingCheckBehaviour._check_offence_status` crashed the slashing background app while iterating the offence status restored by `sync_db_and_slashing`. In both cases the `ValueError` propagated as an `AEAActException` and killed the AEA. The two call sites now catch the `ValueError` and degrade gracefully -- `_get_reset_params` falls back to the default reset parameters (`None`), mirroring its `default`/`on_startup` branch and `TmManager._get_reset_params`; `_check_offence_status` skips the check until the next transition -- and each emits a single `WARNING` (the `_check_offence_status` one latched, since that background app retries every tick). No API changes (#2521).
