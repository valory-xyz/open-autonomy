...
- Bumps `open-aea` `2.2.8` → `2.2.9` across `pyproject.toml`, `setup.py`, `tox.ini`, `skaffold.yaml`, both Dockerfiles' `AEA_VERSION` arg, `deployments/Dockerfiles/autonomy-user/requirements.txt`, the 33 package YAMLs that pin `open-aea-*` dependencies, the two doc snippets (`docs/guides/define_agent.md`, `docs/advanced_reference/commands/autonomy_build-image.md`), and `CLAUDE.md`. Pulls in [open-aea 2.2.9](https://github.com/valory-xyz/open-aea/releases/tag/v2.2.9): `EthereumApi.build_transaction` now propagates the sender as `from` into the built `tx_params`, so `update_with_gas_estimate` calls `eth_estimateGas` with the actual sender instead of `msg.sender = 0x0`. This was producing spurious reverts during pre-flight gas estimation for ERC20 / marketplace flows that depend on `msg.sender` (e.g. `approve` reverting with "ERC20: approve from the zero address", `requestBatch` reverting with "ERC20: transfer amount exceeds allowance" even though the signed transaction would succeed) ([#925](https://github.com/valory-xyz/open-aea/pull/925)). No API or wire-format changes; `autonomy/constants.py` `ABSTRACT_ROUND_ABCI_SKILL_WITH_HASH` resynced to the relocked IPFS hash; package hashes regenerated via `autonomy packages lock` and `poetry.lock` regenerated to match.
