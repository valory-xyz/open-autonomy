...
class TxHashRound(PriceEstimationAbstractRound):
    ...
    def tx_hash(self, payload: TransactionHashPayload) -> None:
        ...
        if sender != cast(PeriodState, self.period_state).safe_sender_address:
    ...
    def check_tx_hash(self, payload: TransactionHashPayload) -> bool:
        ...
        """
        sender = payload.sender
        sender_in_participants = sender in self.period_state.participants
        sender_not_designated = sender == self.period_state.safe_sender_address
    ...
    def end_block(self) -> Optional[Tuple[BasePeriodState, AbstractRound]]:
        ...
        if self.tx_hash_is_set:
            state = self.period_state.update(safe_tx_hash=self.transaction_hash)
