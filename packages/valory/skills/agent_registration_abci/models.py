...
"""This module contains the shared state for the agent registration ABCI application."""
...
class SharedState(BaseSharedState):
    ...
    def setup(self) -> None:
        """Set up."""
        super().setup()
        AgentRegistrationAbciApp.event_to_timeout[
            Event.ROUND_TIMEOUT
        ] = self.context.params.round_timeout_seconds
        AgentRegistrationAbciApp.event_to_timeout[Event.RESET_TIMEOUT] = (
            self.context.params.round_timeout_seconds * MULTIPLIER
        )
        AgentRegistrationAbciApp.event_to_timeout[Event.VALIDATE_TIMEOUT] = (
            self.context.params.retry_timeout * self.context.params.retry_attempts
            + MARGIN
        )
        AgentRegistrationAbciApp.event_to_timeout[Event.DEPLOY_TIMEOUT] = (
            self.context.params.retry_timeout * self.context.params.retry_attempts
            + MARGIN
        )
        AgentRegistrationAbciApp.event_to_timeout[Event.RESET_AND_PAUSE_TIMEOUT] = (
