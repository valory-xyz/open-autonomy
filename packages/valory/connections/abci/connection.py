...
class ABCIServerConnection(Connection):  # pylint: disable=too-many-instance-attributes
    ...
    def _process_tendermint_params(self) -> None:
        ...
        tendermint_logger = cast(logging.Logger, self.logger).getChild("tendermint")
        for handler in tendermint_logger.handlers[:]:
            tendermint_logger.removeHandler(handler)
            handler.close()
        self.node = TendermintNode(self.params, tendermint_logger)
