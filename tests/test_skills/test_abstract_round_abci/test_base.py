...
class TestBaseSynchronizedData:
    ...
    def setup(self) -> None:
        ...
        self.base_synchronized_data = BaseSynchronizedData(
            db=AbciAppDB(
                initial_round=0, initial_data=dict(participants=self.participants)
            )
        )
    ...
    def test_participants_getter_negative(self) -> None:
        ...
        base_synchronized_data = BaseSynchronizedData(
            db=AbciAppDB(initial_round=0, initial_data={})
        )
    ...
    def test_update(self) -> None:
        ...
        expected = BaseSynchronizedData(
            db=AbciAppDB(initial_round=0, initial_data=dict(participants=participants))
        )
    ...
    def test_participants_list_is_empty(
        ...
        base_synchronized_data = BaseSynchronizedData(
            db=AbciAppDB(initial_round=0, initial_data=dict(participants={}))
        )
    ...
    def test_all_participants_list_is_empty(
        ...
        """Tets when participants list is set to zero."""
        base_synchronized_data = BaseSynchronizedData(
            db=AbciAppDB(initial_round=0, initial_data=dict(all_participants={}))
        )
        with pytest.raises(ValueError, match="List participants cannot be empty."):
...
class TestAbciApp:
    ...
    def test_cleanup(self) -> None:
        ...
        dummy_state = BaseSynchronizedData(
            db=AbciAppDB(
                initial_round=0, initial_data=dict(participants=max_participants)
            )
        )
