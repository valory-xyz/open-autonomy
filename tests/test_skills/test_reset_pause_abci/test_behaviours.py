...
from packages.valory.skills.reset_pause_abci.handlers import HttpHandler, SigningHandler
from packages.valory.skills.reset_pause_abci.rounds import Event, PeriodState
...
class ResetPauseAbciFSMBehaviourBaseCase(BaseSkillTestCase):
    """Base case for testing PauseReset FSMBehaviour."""

    path_to_skill = Path(ROOT_DIR, "packages", "valory", "skills", "reset_pause_abci")

    reset_pause_abci_behaviour: ResetPauseABCIConsensusBehaviour
    ...
    def setup(cls, **kwargs: Any) -> None:
        ...
        cls.reset_pause_abci_behaviour = cast(
            ResetPauseABCIConsensusBehaviour,
        ...
        cls.reset_pause_abci_behaviour.setup()
        cls._skill.skill_context.state.setup()
        cls._skill.skill_context.state.period.end_sync()
        assert (
            cast(BaseState, cls.reset_pause_abci_behaviour.current_state).state_id
            == cls.reset_pause_abci_behaviour.initial_state_cls.state_id
        )
    ...
    def mock_http_request(self, request_kwargs: Dict, response_kwargs: Dict) -> None:
        ...
        self.reset_pause_abci_behaviour.act_wrapper()
    ...
    def end_round(
        ...
        current_state = cast(BaseState, self.reset_pause_abci_behaviour.current_state)
        ...
        self.reset_pause_abci_behaviour._process_current_round()
    ...
    def _test_done_flag_set(self) -> None:
        ...
        with mock.patch.object(
            self.reset_pause_abci_behaviour.context.state, "_period"
        ) as mock_period:
...
class TestResetAndPauseBehaviour(ResetPauseAbciFSMBehaviourBaseCase):
    ...
    def test_pause_and_reset_behaviour(
        ...
        self.fast_forward_to_state(
            behaviour=self.reset_pause_abci_behaviour,
        ...
        assert (
            cast(
                BaseState,
                cast(BaseState, self.reset_pause_abci_behaviour.current_state),
            ).state_id
            == self.behaviour_class.state_id
        )
        self.reset_pause_abci_behaviour.context.params.observation_interval = 0.1
        self.reset_pause_abci_behaviour.act_wrapper()
        time.sleep(0.3)
        self.reset_pause_abci_behaviour.act_wrapper()
    ...
    def test_reset_behaviour(
        ...
        self.reset_pause_abci_behaviour.current_state.pause = False  # type: ignore
        assert (
            cast(
                BaseState,
                cast(BaseState, self.reset_pause_abci_behaviour.current_state),
