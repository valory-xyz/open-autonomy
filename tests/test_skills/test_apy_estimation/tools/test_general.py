...
class TestGeneral:
    ...
    def test_gen_unix_timestamps(monkeypatch):
        ...
        monkeypatch.setattr(time, 'time', lambda: day_in_unix * n_months_to_check * (days_in_month + 1))
    ...
    @staticmethod
    @pytest.mark.parametrize('test_path', ('', 'file.extension', 'folder/file.extension'))
    def test_create_pathdirs(tmp_path, test_path):
        ...
        n_files_in_folder = len([name for name in os.listdir(tmp_path) if os.path.isfile(name)])
        n_folders_in_folder = len(
            [name for name in os.listdir(tmp_path) if os.path.isdir(os.path.join(tmp_path, name))]
        )
