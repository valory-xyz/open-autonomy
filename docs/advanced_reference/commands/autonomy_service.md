...
: ERC20 token to use for activating the service.
...
If an ERC20 token was used as bonding token you'll have to provide the token address using `--token` flag:

```bash
autonomy service activate 42 --key my_key.txt --token <token_address>
```

Make sure your account holds enough tokens for activating the service:
...
`--token ERC20_TOKEN_ADDRESS`
: ERC20 token to use as bond for the agent instances.
...
Make sure your account holds enough tokens to pay for the agent bonds.
...
To terminate a service with ID 42 in the Autonolas Protocol:
