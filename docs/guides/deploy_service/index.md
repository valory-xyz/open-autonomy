...
5. **Run the service.** Navigate to the deployment environment folder (`./abci_build`) and run the deployment locally.
    ...
    === "Kubernetes"
        ...
        We show how to run the deployment on your local machine using kind. These instructions might vary depending on you
...
3. **Deploy the service.** Execute the following command:
    ...
    === "Kubernetes"
        ...
        The Kubernetes deployment will be built for the agents whose keys are defined in the `keys.json` file. You need to run the Kubernetes deployment manually, as it is currently not supported by the framework.
