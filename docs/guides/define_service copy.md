Given an existing agent, either developed by yourself on the previous step, or fetch from the remote registry, the next step consists in defining the agent service and its configuration options.
