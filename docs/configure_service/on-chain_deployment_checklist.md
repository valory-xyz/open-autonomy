...
=== "service.yaml"
    ...
    ```
!!! warning "Important"
    Recall that when [deploying an on-chain service](../guides/deploy_service/index.md#on-chain-deployment) using `autonomy deploy from-token`, a number of arguments (under `setup`) are overridden with the values registered in the Autonolas Protocol:
    ```yaml
    (...)
    models:
      params:
