...
<a id="packages.valory.skills.transaction_settlement_abci.tests.test_behaviours.mock_yield_and_return"></a>
...
<a id="packages.valory.skills.transaction_settlement_abci.tests.test_behaviours.test_skill_public_id"></a>
...
<a id="packages.valory.skills.transaction_settlement_abci.tests.test_behaviours.TransactionSettlementFSMBehaviourBaseCase.ffw_signature"></a>

#### ffw`_`signature

```python
def ffw_signature(db_items: Optional[Dict] = None) -> None
```
...
```python
class TestTransactionSettlementBaseBehaviour(
        TransactionSettlementFSMBehaviourBaseCase)
```
...
@pytest.mark.parametrize(
    ...
    (
        (
            MagicMock(performative=ContractApiMessage.Performative.ERROR,
                      message="GS026"),
            None,
...
def test__get_tx_data(message: ContractApiMessage, tx_digest: Optional[str],
                      rpc_status: RPCResponseStatus, expected_data: TxDataType,
                      replacement: bool, monkeypatch: MonkeyPatch) -> None
```
...
def test_get_gas_price_params(tx_body: dict,
...
```python
class TestSelectKeeperTransactionSubmissionBehaviourA(
...
def test_select_keeper(final_verification_status_mock: mock.PropertyMock,
                       keeper_retries_mock: mock.PropertyMock,
                       keepers_mock: mock.PropertyMock, keepers: Deque[str],
                       keeper_retries: int, blacklisted_keepers: Set[str],
                       final_verification_status: VerificationStatus) -> None
```

Test select keeper agent.

<a id="packages.valory.skills.transaction_settlement_abci.tests.test_behaviours.TestSelectKeeperTransactionSubmissionBehaviourB.test_select_keeper_tx_pending"></a>

#### test`_`select`_`keeper`_`tx`_`pending
...
@pytest.mark.skip
def test_select_keeper_tx_pending(_: mock.PropertyMock,
...
@mock.patch.object(SkillContext,
                   ...
                   new_callable=mock.PropertyMock)
def test_sender_act(
    agent_address_mock: mock.PropertyMock, resubmitting: bool,
...
<a id="packages.valory.skills.transaction_settlement_abci.tests.test_behaviours.TestFinalizeBehaviour.test_sender_act_tx_data_contains_tx_digest"></a>

#### test`_`sender`_`act`_`tx`_`data`_`contains`_`tx`_`digest

```python
def test_sender_act_tx_data_contains_tx_digest() -> None
```
...
```python
class TestValidateTransactionBehaviour(
        TransactionSettlementFSMBehaviourBaseCase)
```
...
```python
class TestCheckTransactionHistoryBehaviour(
        TransactionSettlementFSMBehaviourBaseCase)
```
...
def test_check_tx_history_behaviour(verified: bool, status: int,
                                    hashes_history: str,
...
<a id="packages.valory.skills.transaction_settlement_abci.tests.test_behaviours.TestCheckTransactionHistoryBehaviour.test_check_tx_history_behaviour_negative"></a>

#### test`_`check`_`tx`_`history`_`behaviour`_`negative
...
def test_check_tx_history_behaviour_negative(verified: bool, status: int,
                                             hashes_history: str,
                                             revert_reason: str) -> None
```
...
<a id="packages.valory.skills.transaction_settlement_abci.tests.test_behaviours.TestCheckLateTxHashesBehaviour"></a>

## TestCheckLateTxHashesBehaviour Objects
...
<a id="packages.valory.skills.transaction_settlement_abci.tests.test_behaviours.TestCheckLateTxHashesBehaviour.test_check_tx_history_behaviour"></a>

#### test`_`check`_`tx`_`history`_`behaviour

```python
def test_check_tx_history_behaviour() -> None
```
...
```python
class TestSynchronizeLateMessagesBehaviour(
        TransactionSettlementFSMBehaviourBaseCase)
```
