...
<a id="autonomy.data.contracts.agent_registry.contract.AgentRegistryContract.get_events"></a>
...
<a id="autonomy.data.contracts.agent_registry.contract.AgentRegistryContract.get_hash"></a>

#### get`_`hash

```python
@classmethod
def get_hash(cls, ledger_api: LedgerApi, contract_address: str,
             agent_id: int) -> JSONLike
```

Retrieve an operator given its agent instance.

<a id="autonomy.data.contracts.agent_registry.contract.AgentRegistryContract.authenticate_sender"></a>

#### authenticate`_`sender

```python
@classmethod
def authenticate_sender(cls, ledger_api: LedgerApi, contract_address: str,
                        sender_address: str,
                        mech_address: str) -> Dict[str, Any]
```
