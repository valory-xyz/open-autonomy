...
class Package:  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        ...
        if self.type == "customs":
            print(f"Package: skipping {self.package_id} as it is a customs package")
...
class PackageHashManager:
    ...
    def __init__(
        ...
        """Constructor"""
        self.skip_hashes = skip_hashes or []
        ...
        for url in package_json_urls or []:
            ...
            self.packages.extend(
                [
                    Package(key, value, root_dir, True)
                    for key, value in new_packages.items()
                ]
            )
...
def check_ipfs_hashes(  # pylint: disable=too-many-locals,too-many-statements
    ...
    for md_file in all_md_files:
        ...
        for match in [
            ...
            if package_hash in skip_hashes:
                continue
    ...
    package_list_file = root_dir / "docs" / "package_list.md"
...
def check_doc_hashes(
    ...
    root_dir = Path.cwd()
    paths = [Path(p) for p in doc_paths] if doc_paths else [Path("docs")]
    check_ipfs_hashes(
        root_dir=root_dir,
        paths=paths,
        fix=fix,
